...
BEGIN_BEHAVIOR
    FINISH_COND
        Message(msg_skin_act_end) && !Message(msg_skin_act_begin)
    FINISH_ACTION
        GetPlayAnim()->SetAnim(GetAnimation(anim_default));
    CONDITION
        Message(msg_skin_act_begin)
    ACTION gData.ScriptMgr->FlushTimedMessagesTo(this, msg_skin_act_end);
        Animation_ZHdl l_Anim = GetAnimation(anim_action);
        GetPlayAnim()->SetAnim(l_Anim);
        SendTimed(msg_skin_act_end, this, l_Anim->GetMaxTime());
END_BEHAVIOR
